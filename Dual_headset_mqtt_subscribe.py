...
# CSV file setup
csv_files = {participant: f'headset_data_{participant}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv' for participant in participants}
...
def log_data(participant, headset, key, value):
    ...
    with open(csv_file, mode='a', newline='') as file:
        ...
        if isinstance(value, dict):
            row = [datetime.now(), participant, headset] + list(value.values())
            writer.writerow(row)
        else:
...
def on_message(client, userdata, msg):
    global headset1_data, headset2_data, meditation_count
    ...
    if headset1_data and headset2_data:
        meditation1 = headset1_data[2]  # Meditation value based on provided order
        meditation2 = headset2_data[2]  # Meditation value based on provided order
        participant1 = participants[0]
        participant2 = participants[1] if len(participants) > 1 else participants[0]
