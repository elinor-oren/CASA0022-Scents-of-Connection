...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        participant_index = participants[0] if "headset1" in topic else (participants[1] if len(participants) > 1 else participants[0])
        if data.get("Signal Strength", 1) == 0:  # Check if signal strength is 0
            valid_packets_received[participant_index] += 1
            headset_data[participant_index] = data
