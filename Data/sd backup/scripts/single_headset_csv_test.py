...
def breathing_effect(times, interval):
    steps = 200  # Number of steps for breathing in and out
    hold_time = 1  # Hold time at peak brightness
    for _ in range(times):
        for step in range(steps):
            brightness = (math.sin(step / float(steps) * math.pi) + 1) / 2  # Breathing effect
            for i in range(1, 22):
        ...
        for step in range(steps):
            brightness = (math.sin((steps - step) / float(steps) * math.pi) + 1) / 2  # Breathing effect
            for i in range(1, 22):
                pixels[i] = tuple(int(brightness * c) for c in teal)
            pixels.show()
            time.sleep(interval / steps)
...
'''
def breathing_effect(times, interval):
    steps = 50  # Number of steps for breathing in and out
    for _ in range(times):
        for step in range(steps):
            brightness = (math.sin(step / float(steps) * math.pi) + 1) / 2  # Breathing effect
            for i in range(1, 22):
...
def on_message(client, userdata, msg):
    ...
    if valid_packets_received >= 3 and not experiment_started:
        ...
        pixels.fill((0, 0, 0, 0))
        pixels.show()
        breathing_effect(1, 2)  # Maintain a breathing effect once the experiment has started
        pixels.fill((0, 0, 0, 0))
        pixels.show()
...
try:
    while True:
        if valid_packets_received < 3:
            ...
            breathing_effect(1, 2)  # Slow breathing effect after the experiment has started
        time.sleep(1)
