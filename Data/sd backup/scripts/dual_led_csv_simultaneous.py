...
HEADSET2_LEDS = range(42, 60)  # LEDs 42-59 for headset 2
        ...
        for i, participant in enumerate(participants):
            ...
            if headset:
                ...
                if i == 0:
                    ...
                    current_state_headset1 = apply_state(new_state1, HEADSET1_LEDS)
                elif i == 1:
                    new_state2 = calculate_gradient_effect_headset2(meditation)
                    current_state_headset2 = apply_state(new_state2, HEADSET2_LEDS)

def apply_state(new_colors, led_range):
    """Apply the new state directly without smooth transition."""
    for i, led_index in enumerate(led_range):
        if i < len(new_colors):
            pixels[led_index] = new_colors[i]
    pixels.show()
    return new_colors
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if data[0] == 0:  # Assuming valid data has 'Signal Strength' of 0
            ...
            # Update LEDs immediately after receiving data
            update_leds()
    ...
    if all(count > 4 for count in valid_packets_received.values()) and not experiment_started:
        ...
        log_data(participants[0], "Experiment", "Started")
...
try:
    while True:
        if not experiment_started:
            chaser_effect()
        time.sleep(0.1)  # Reduced sleep to allow more frequent LED updates
