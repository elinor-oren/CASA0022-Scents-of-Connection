...
# Define color and brightness levels for each meditation range
color_levels = {
...
# CSV file setup
csv_file = f'{participant}_headset_data_{datetime.now().strftime("%Y%m%d_%H%M")}.csv'
...
def rainbow_cycle(wait):
    ...
    with lock:
        ...
        for j in range(255):  # Cycle through each color position
            for i in range(22, 59):  # Apply the color to each LED
...
def fade_transition(start_led, end_led, target_color, steps=20):
    """Fade transition to target color for LEDs between start_led and end_led."""
    for step in range(steps):
        for i in range(start_led, end_led + 1):
            if 0 <= i < num_pixels:  # Ensure we stay within range
                current_color = pixels[i]
                color = gradient_color(current_color, target_color, step, steps - 1)
                pixels[i] = color
        pixels.show()
        time.sleep(0.01)
...
def set_leds(meditation):
    ...
    if meditation <= 20:
        fade_transition(22, 26, start_color)
        fade_transition(57, 60, start_color)
    elif meditation <= 40:
        fade_transition(22, 26, start_color)
        fade_transition(57, 60, start_color)
        fade_transition(27, 30, end_color)
        fade_transition(53, 56, end_color)
    elif meditation <= 60:
        fade_transition(22, 30, start_color)
        fade_transition(53, 60, start_color)
        fade_transition(31, 34, end_color)
        fade_transition(49, 52, end_color)
    elif meditation <= 80:
        fade_transition(22, 34, start_color)
        fade_transition(49, 60, start_color)
        fade_transition(35, 38, end_color)
        fade_transition(45, 48, end_color)
    else:
        fade_transition(22, 38, start_color)
        fade_transition(45, 60, start_color)
        fade_transition(39, 41, end_color)
        fade_transition(42, 44, end_color)
